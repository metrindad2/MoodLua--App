...
Este é um aplicativo Next.js para monitoramento do ciclo menstrual, criado pelo Firebase Studio.
...
- **`src/context/cycle-data-context.tsx`**: Este é o "cérebro" do lado do cliente. Ele gerencia todos os dados do aplicativo (perfil, logs diários) e os salva no `localStorage` do navegador, para que as informações não se percam ao fechar a aba.
...
4. **Adicione navegação**: No componente `src/components/app-shell.tsx`, adicione um link para sua nova página no menu de navegação.
